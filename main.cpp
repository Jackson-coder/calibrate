...
int main(int argc, char **argv)
{
    ...
    cv::CommandLineParser parser(argc, argv,
                                 "{@arg1|/home/camera_calibrate/left05.jpg|}{@arg2|/home/camera_calibrate/right05.jpg|}{help h||}{algorithm|bm|}{max-disparity|256|}{blocksize|5|}{no-display||}{color||}{scale|1|}{i|/home/camera_calibrate/intrinsics.yml|}{e|/home/camera_calibrate/extrinsics.yml|}{o|record.jpg|}{p|record|}");
    if (parser.has("help"))
    {
    ...
    if (!point_cloud_filename.empty())
    {
        ...
        vector<Point2f> testoutputPoints2D;
        vector<Point2f> testoriginPoints2D;
        for (int y = 0; y < xyz.rows; y++)
        {
            ...
            for (int x = 0; x < xyz.cols; x++)
            {
                ...
                e.back() = 1;
                if (testoutputPoints2D.size() < 10)
                    testoutputPoints2D.push_back(Point2f(x, y));
                //printf("%d %d\n",y,x);
            }
            error.push_back(e);
        }
        ...
        //测试点的三维信息
        for (auto poi : testoutputPoints2D)
        {
        ...
        //             // {
        //             //     if (pts_out[h] == Point2f(x,y))
        //             //         ptt = pts_in[h];
        //             // }
        //             // error[y][x] = fabs(ptt.y - imagePoints[t].y) + fabs(ptt.x - imagePoints[t].x); //error[y][x]
        //             Point2f P = Point2f(x, y);
        //             error[y][x] = fabs(y - imagePoints[t].y) + fabs(x - imagePoints[t].x);
        //             // error[y][x] = fabs(pts[P].y - imagePoints[t].y) + fabs(pts[P].x - imagePoints[t].x); //error[y][x]
        //             if (x == outputPoint.x && y == outputPoint.y) //(x == 289 && y == 477)
        //             {
        //                 // circle(img1, Point((int)imagePoints[t].y, (int)imagePoints[t].x), 10, Scalar(20, 100, 100), 2);
        //                 cout << Point((int)imagePoints[t].x, (int)imagePoints[t].y) << P << endl;
        //             }
        //             t++;
        //             //cout << error[y][x] << endl;
        //         }
        ...
        int filter=0;
        for (int tt = 0; tt < testoriginPoints2D.size(); tt++)
        {
            while(testoutputPoints2D[filter]==Point2f(0,0)) filter++;
            
            circle(img1, testoutputPoints2D[filter], 20, Scalar(20, 100, 100), 2);
            cout << testoriginPoints2D[tt] << ' ' << testoutputPoints2D[filter] << ' ' << imagePoints[filter] << ' ' << fabs(testoriginPoints2D[tt].y - imagePoints[filter].y) + fabs(testoriginPoints2D[tt].x - imagePoints[filter].x) << endl;
            filter++;
        }
